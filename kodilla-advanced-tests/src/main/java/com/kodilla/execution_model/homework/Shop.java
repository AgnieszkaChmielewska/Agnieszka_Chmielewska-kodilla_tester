...
public class Shop {
    ...
    public void addOrder(Order order) {
        if (order.getPrice() < 0 || order.getPrice() == 0) {
            return;
        } else if (order.getOrderDate().isBefore(LocalDate.now(clock))) {
            return;
        } else {
...
//        return ordersInRangeOfDate;
        ...
        for (Order order : orders) {
            ...
            if (order.getOrderDate().isAfter(orderDateFrom) && order.getOrderDate().isBefore(orderDateTo)) {
                result.add(order);
...
//    }
    ...
    public static void main(String[] args) {
        Shop shop = new Shop(Clock.systemUTC());
